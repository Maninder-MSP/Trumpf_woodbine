# FlexMod_ICP-CONDigIO.py

# Description
# ICP-CON Digital IO Module - The Digital IO module services a dedicated device having digital inputs and outputs.
# This version of it supports the ICP-DAS, ICP-CON (ET-7052) having 8 Inputs and 8 Outputs, and is
...
class Module():
    def __init__(self, uid, queue):
        ...
        self.manufacturer = "ICP-CON"
        self.model = ""
        self.options = ""
        self.version = "-"
        self.serial = "-"                                                                           # This can be replaced with the device serial number later
        self.website = "/Mod/FlexMod_ICP-CONDigIO"                                                  # This is the template name itself
        ...
        self.override = False
        self.device_reboot = False
        self.inputs = [self.uid, False, [False]*16]                                               # 8 outputs and 8 reserved
        self.outputs = [self.uid, self.enabled, [0]*25]
        self.outputs_debug = [False]*8
        self.outputs_debug_count = [0, 0, 0, 0, 0, 0, 0, 0]
        self.heartbeat = 0
    ...
    def process(self):
        ...
        if self.tcp_client is None:
                ...
                if self.dbData["digio_ipaddr_local"] != "0.0.0.0":
                    try:
                        ...
                        if self.tcp_client.connect() is True:
                            ...
                            # Clear the outputs # the same as ADAM-6060
                            clr_bits = [0, 0, 0, 0, 0, 0, 0, 0]
                            self.tcp_client.write_coils(0, clr_bits, unit=1)
            ...
            rr = []
            try:
                rr = self.tcp_client.read_holding_registers(260, 1, unit=1)
                if rr.isError():
                    self.tcp_timeout += 1
                    return
                else:
                    self.tcp_timeout = 0
                    rr = self.tcp_client.read_holding_registers(260, 1, unit=1)
                    self.name = "ET-" + str(hex(rr.registers[0]).lstrip("0x"))
            ...
            rr = []
            try:
                rr = self.tcp_client.read_input_registers(100, 1, unit=1)
                if rr.isError():
                    self.tcp_timeout += 1
                    return
                else:
                    self.tcp_timeout = 0
                    self.input_qty = rr.registers[0]
            except Exception as e:
                print("ICP-CON DIGIO: " + str(e))
                self.tcp_timeout += 1
                return
            ...
            rr = []
            try:
                rr = self.tcp_client.read_input_registers(110, 1, unit=1)
                if rr.isError():
                    self.tcp_timeout += 1
                    return
                else:
                    self.tcp_timeout = 0
                    self.output_qty = rr.registers[0]
            except Exception as e:
                print("ICP-CON DIGIO: " + str(e))
                self.tcp_timeout += 1
                return
            ...
            rr = []
            try:
                rr = self.tcp_client.read_input_registers(151, 1, unit=1)
                if rr.isError():
                    self.tcp_timeout += 1
                    return
                else:
                    self.tcp_timeout = 0
                    fw_ver_dot = hex(rr.registers[0]).lstrip("0x")
                    self.version ='.'.join(fw_ver_dot[i:i + 1] for i in range(0, len(fw_ver_dot), 1))
            ...
            rr = []
            try:
                rr = self.tcp_client.read_discrete_inputs(0, self.input_qty, unit=1)
                ...
                if rr.isError():
                    ...
                    for x in range(self.input_qty):  # Invert the input data bits so pulling to ground == True
            ...
            rr = []
            try:
                rr = self.tcp_client.read_coils(0, 8, unit=1)
                ...
                if rr.isError():
                    ...
                    for x in range(8):  # Check if the output states match the requested state
                        # Find the Fault by Sophie same as ADAM-6060
                        if ((rr.bits[x] is True) and (((self.inputs[2][5] >> (x * 2)) & 0x03) != 0x01)) or ((rr.bits[x] is False) and (((self.inputs[2][5] >> (x * 2)) & 0x03) != 0x00)):
                            if self.outputs_debug_count[x] <= 7:
                    ...
                    if change_op_state:
                        self.tcp_client.write_coils(0, rr.bits, unit=1)  # Update on state change only to minimise traffic
            ...
            # ICP-CON device reboot command
            if self.device_reboot:
                try:
                    self.tcp_client.write_coil(133, 1, unit=1)
                    self.device_reboot = False
                    self.tcp_timeout = 0
                except Exception as e:
                    print("ICP-CON DIGIO: " + str(e))
            ...
            self.outputs[2][1] = 0
            self.outputs[2][2] = self.input_qty
            self.outputs[2][3] = self.output_qty
            self.outputs[2][4] = self.digio_input_bits & 0xFF           # 8-bit DI from ICP-CON
            self.outputs[2][5] = self.digio_output_bits & 0xFFFF        # 8-bit DO from ICP-CON
    ...
    def set_page(self, page, form):                                                                       # Respond to GET/POST requests
        ...
        if page == self.website + "_(" + str(self.uid) + ")":                                    # It was a page request for a numbered device (POST)
            ...
            for control in form:
                ...
                elif "digio_output_6_test" in form:                                 # for more than 6 DO
                    if (self.inputs[2][5] >> 12) & 0x03 == 0x01:
                        self.inputs[2][5] &= ~(0x03 << 12)
                    elif (self.inputs[2][5] >> 12) & 0x03 == 0x00:
                        self.inputs[2][5] |= (0x01 << 12)
                    self.actions.insert(0, Actions.CTRL_OUTPUT6_CHANGE.value)
                elif "digio_output_7_test" in form:
                    if (self.inputs[2][5] >> 14) & 0x03 == 0x01:
                        self.inputs[2][5] &= ~(0x03 << 14)
                    elif (self.inputs[2][5] >> 14) & 0x03 == 0x00:
                        self.inputs[2][5] |= (0x01 << 14)
                    self.actions.insert(0, Actions.CTRL_OUTPUT7_CHANGE.value)
                elif "digio_reboot_test" in form:
                    self.device_reboot = not self.device_reboot
                    self.actions.insert(0, Actions.DEVICE_REBOOTED.value)
...
class Faults(Enum):
    ...
    IO_OP6_FAULT = 9                                                                                # "Fault: Digital IO - Output 6"
    IO_OP7_FAULT = 10                                                                               # "Fault: Digital IO - Output 7"
...
class Actions(Enum):
    ...
    CTRL_OUTPUT6_CHANGE = 9
    CTRL_OUTPUT7_CHANGE = 10
    DEVICE_REBOOTED = 11
