...
class Module():
    def __init__(self, uid, queue):
        ...
        self.version = "-"
        self.serial = "-"
        self.website = "/Mod/FlexMod_BACNetACMeter"
        ...
        self.ac_meter_total_active_power = 0                                                        # Currently this is the only data point available for the EPH Building Meter
        ...
        if self.tcp_client is None:
            ...
            if "ac_meter_ipaddr_local" in self.dbData:
                if self.dbData["ac_meter_ipaddr_local"] != "0.0.0.0":
                    ...
                    try:
                        self.tcp_client = modbus_client(self.dbData["ac_meter_ipaddr_local"], port=504)     # <--- Use port 504 on the BACnet/Modbus bridge to not conflict with SCADA on port 502
            ...
            try:
                ...
                if rr.isError():
                    ...
                    self.tcp_timeout = 0
                    self.ac_meter_total_active_power = self.twos_comp_to_int(rr.registers[1]) / 10

            except Exception as e:
                print("BACNet AC Meter 2: " + str(e))
                self.tcp_timeout += 1
                return
