...
class Module():
    def __init__(self, uid, queue):
        ...
        # Trumpf called Alarm to fault
        self.alarm_clear_timeout = 15
        self.alarm_clear_timeout_counter = 0
    ...
    def process(self):
        ...
        if self.tcp_client is None:
            ...
            # Read the live Alarms
            try:
                rr = self.tcp_client.read_holding_registers(2808, 2, timeout=0.25)
                if rr.isError():
                    self.tcp_timeout += 1
                    return
                else:
                    # Trumpf called Alarm to fault
                    self.alarms = rr.registers[1]
            ...
            # Read the live Warnings
            try:
                rr = self.tcp_client.read_holding_registers(2401, 2, timeout=0.25)
                if rr.isError():
                    self.tcp_timeout += 1
                    return
                else:
